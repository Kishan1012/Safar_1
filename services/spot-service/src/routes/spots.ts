...
// Try to avoid connecting to Redis if not provided in dev
const isDev = !process.env.REDIS_URL || process.env.REDIS_URL.includes('localhost');

// We proxy Redis to avoid it attempting to connect and throwing connection refused
const redis = isDev
  ? (new Proxy({}, { get: () => () => Promise.resolve() }) as unknown as Redis)
  : new Redis(process.env.REDIS_URL!);

// Dummy queue for local dev if redis is mocked
const importQueue = isDev
  ? ({ add: async (name: string, data: any) => console.log(`[MOCK REDIS] Added job to queue: ${name}`, data) } as unknown as Queue)
  : new Queue('spot-import', { connection: redis });
